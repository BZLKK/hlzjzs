      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                const Text("自动首行缩进: ", style: TextStyle(fontSize: 12, color: Colors.grey)),
                Switch(
                  value: provider.autoIndent,
                  onChanged: (val) => provider.setAutoIndent(val),
                ),
                const Text(" (换行自动加两个空格)", style: TextStyle(fontSize: 10, color: Colors.grey)),
              ],
            ),
            const Divider(),
            _buildDropdown(
