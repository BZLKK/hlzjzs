...
final GlobalKey<WritingEditorState> writingEditorKey = GlobalKey<WritingEditorState>();
...
class MyApp extends StatelessWidget {
  ...
  ThemeMode _getThemeMode(String mode) {
    switch (mode) {
      case "明亮模式": return ThemeMode.light;
      case "暗黑模式": return ThemeMode.dark;
...
class _MainPageState extends State<MainPage> with WindowListener {
                  ...
                  child: Material(
                    ...
                    child: Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          const Icon(Icons.self_improvement, color: Colors.white, size: 28),
  ...
  PreferredSizeWidget _buildAppBar(BuildContext context) {
    ...
    return AppBar(
      title: PopupMenuButton<String>(
        ...
        itemBuilder: (context) => [
          const PopupMenuItem(value: "settings", child: ListTile(leading: Icon(Icons.settings), title: Text("个性化设置"))),
          const PopupMenuItem(value: "export", child: ListTile(leading: Icon(Icons.rocket_launch), title: Text("导出书稿"))),
        ...
        child: const Row(
          ...
          children: [
            ...
            SizedBox(width: 8),
            Text("菜单", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          ],
        ),
      ),
      actions: [
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Column(
          ...
          children: [
            Text("HL作家助手 v1.0", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
            SizedBox(height: 8),
            Text("这是一款专注于沉浸式打字的无干扰写作软件。"),
            SizedBox(height: 12),
            Text("作者邮箱: hzlkkk@gmail.com", style: TextStyle(color: Colors.blue)),
          ],
        ),
        actions: [
          TextButton(
  ...
  void _showExportDialog(BuildContext context, WritingProvider provider) {
    if (provider.currentBookName == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("请先选择书籍")));
      return;
    }
    showDialog(context: context, builder: (context) => ExportDialog(provider: provider));
  }
  ...
  void _showHistory(BuildContext context, WritingProvider provider) {
    if (provider.currentBookName == null || provider.currentChapterName == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("请先选择章节")));
      return;
    }
    showDialog(context: context, builder: (context) => HistoryDialog(provider: provider));
  }
}
